...
class Log private constructor() {
    ...
    companion object {
        ...
        fun v(contents: Any?) {
            print(Log.VERBOSE, logTag, contents?.toString() ?: NULL)
        }

        fun v(tag: String, contents: Any?) {
            print(Log.VERBOSE, tag, contents?.toString() ?: NULL)
        }

        fun d(contents: Any?) {
            print(Log.DEBUG, logTag, contents?.toString() ?: NULL)
        }

        fun d(tag: String, contents: Any?) {
            print(Log.DEBUG, tag, contents?.toString() ?: NULL)
        }

        fun i(contents: Any?) {
            print(Log.INFO, logTag, contents?.toString() ?: NULL)
        }

        fun i(tag: String, contents: Any?) {
            print(Log.INFO, tag, contents?.toString() ?: NULL)
        }

        fun a(contents: Any?) {
            print(Log.ASSERT, logTag, contents?.toString() ?: NULL)
        }

        fun a(tag: String, contents: Any?) {
            print(Log.ASSERT, tag, contents?.toString() ?: NULL)
        }

        fun e(contents: Any?) {
            print(Log.ERROR, logTag, contents?.toString() ?: NULL)
        }

        fun e(tag: String, contents: Any?) {
            print(Log.ERROR, tag, contents?.toString() ?: NULL)
        }

        fun w(contents: Any?) {
            print(Log.WARN, logTag, contents?.toString() ?: NULL)
        }

        fun w(tag: String, contents: Any?) {
            print(Log.WARN, tag, contents?.toString() ?: NULL)
        }

        fun log(e: Throwable) {
            print(Log.ERROR, logTag, e.stackTraceToString())
        }

        fun log(tag: String, e: Throwable) {
            print(Log.ERROR, tag, e.stackTraceToString())
        }

        fun json(level: Int = logLevel, tag: String = logTag, json: String?) {
        ...
        private fun print(level: Int, tag: String, msg: String) {/*两重设定，过滤是否需要打印log*/
            ...
            msg.forEach { Log.println(level, tag, "$HORIZONTAL_LINE$it") }
        ...
        fun formatJson(json: String?): String? {
            ...
            try {
                ...
                else if (json.startsWith('[') && json.endsWith(']')) return JSONArray(json).toString(
                    4
                )
                return json
            } catch (e: Exception) {
