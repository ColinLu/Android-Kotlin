...
abstract class BaseAdapter<ITEM>(
    ...
    override fun onAttachedToRecyclerView(recyclerView: RecyclerView) {
        ...
        if (manager is GridLayoutManager) applyGridSpan(manager)
    }

    /**
     * GridLayoutManager是否该位置的 item 需要占满整行（如空布局、尾部布局）
     */
    private fun applyGridSpan(manager: GridLayoutManager) {
        manager.spanSizeLookup = object : GridLayoutManager.SpanSizeLookup() {
            override fun getSpanSize(position: Int): Int {
                return if (!fullSpanViewType(getItemViewType(position))) 1 else manager.spanCount
            }
        }
    }

    /**
     * StaggeredGridLayoutManager是否该位置的 item 需要占满整行（如空布局、尾部布局）
     */
