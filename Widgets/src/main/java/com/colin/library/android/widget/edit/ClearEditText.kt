...
import com.colin.library.android.utils.ONE_SECOND
...
class ClearEditText @JvmOverloads constructor(
    ...
    init {
        ...
        if (mClearDrawable == null) {
            ...
            mClearDrawable = ContextCompat.getDrawable(context, R.drawable.edit_delete)
        }
        mClearDrawable?.setBounds(
            0, 0, mClearDrawable?.intrinsicWidth ?: 0, mClearDrawable?.intrinsicHeight ?: 0
        )
        //默认设置隐藏图标
    ...
    override fun onTextChanged(
        text: CharSequence, start: Int, before: Int, after: Int
    ) {
        if (mHasFocus) {
            //获取焦点设置回之前的图标
            mClearDrawable = ContextCompat.getDrawable(context, R.drawable.edit_delete_gray)
            mClearDrawable?.setBounds(
                0, 0, mClearDrawable?.intrinsicWidth ?: 0, mClearDrawable?.intrinsicHeight ?: 0
            )
            setClearIconVisible(text.isNotEmpty())
